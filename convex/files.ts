...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if files with same name already exist in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
...
export const createFolder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if folder with same name already exist in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingFile) {
      throw new Error(`A ${existingFile.type} with this name already exists`);
    }
...
export const deleteFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Recursivly delete file/folder and all descedants
    const deleteRecursive = async (fileId: Id<"files">) => {
      ...
      // If it's a folder, delete al lchidlren
      if (item.type === "folder") {
      ...
      // Delete storage files if it exist
      if (item.storageId) {
