...
export const processMessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Build system prompt with conversation history (exclude the current processing messsage)
    ...
    // Createa netwrok with single agent
    const netwrok = createNetwork({
      ...
      router: ({ network }) => {
        ...
        // Antrhopic outputs text and tool calls together
    ...
    const result = await netwrok.run(message);
